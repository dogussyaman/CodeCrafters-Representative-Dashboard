...
2. `.env.example` dosyasını `.env.local` olarak kopyalayın ve Supabase bilgilerinizi girin (CodeCrafters ile aynı `NEXT_PUBLIC_SUPABASE_URL` ve `NEXT_PUBLIC_SUPABASE_ANON_KEY`).
3. Supabase’de CodeCrafters şemasına ek olarak `scripts/MT_EXTENSION_SCHEMA.sql` (CodeCrafters repo’sunda `scripts/MT_EXTENSION_SCHEMA.sql`) migration’ını çalıştırın.
